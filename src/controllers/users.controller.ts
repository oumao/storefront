...
const registerUser = async (_req: Request, res: Response) => {
    try {
        ...
        if(userExists){
            res.status(400).json(`User with username ${userObject.username} already exists!!`)
        } else{
            const user = await userStore.createUser(userObject)

            const token = jwt.sign({ id: user.id, username: user.username}, TOKEN_SECRET as string)
