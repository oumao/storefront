...
const getProduct = async (_req: Request, res: Response) => {
    try {
        const product = await productStore.getSingleProduct(_req.params.id as unknown as number)
...
const deleteProduct = async (_req: Request, res: Response) => {
    try {
        ...
        if(!product){
            ...
            const result = await productStore.deleteSingleProduct(_req.params.id as unknown as number)
            res.status(200).json(`Product with id ${_req.params.id} has been deleted!`)
        }
